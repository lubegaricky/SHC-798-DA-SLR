...
From **Model diagnostics plots** , the errors have a relatively constant variance and their expectation can be considered zero (Tuskey-Anscombe plot). The Q-Q plot indicates that the residuals are approximately Gaussian distributed in the central region but exhibit heavy tails (skewness) and some outliers at the extremes.

Although the noticeable presence of extreme positive residuals suggests a right-skewed distribution (departure from normality), the approximation to normality in the center may be sufficient to validate this model.
...
-   The **negative coefficient** for *displ* (-3.5306) supports the belief that cars with smaller engines have better fuel efficiency. As engine size increases, highway fuel economy decreases significantly, with a 1-liter increase in displacement leading to a 3.53 mpg reduction in fuel efficiency, on average.

-   The highly significant **p-values** for both the displ coefficient and the overall model (\< 2e-16) confirm that the negative relationship between engine size and fuel efficiency is not due to random chance. This strengthens the conclusion that engine size is a reliable predictor of fuel efficiency.

-   The **R-squared value** (0.5868) indicates that engine displacement explains a significant portion (58.68%) of the variation in highway fuel economy. However, the remaining 41.32% of variability implies other factors (like, vehicle weight, car transmission type, or car drive type) also influence fuel efficiency.
